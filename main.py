...
def segmentacaoCaractere(original):
    img = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
    lista_palavra_seg = []
    ...
    nitido = cv2.filter2D(borrado, -1, k)
    limiarizado = cv2.threshold(
        nitido, 1, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )[1]

    k = cv2.getStructuringElement(cv2.MORPH_CROSS, (3, 3))
    abertura = cv2.morphologyEx(limiarizado, cv2.MORPH_OPEN, k)
    fechamento = cv2.morphologyEx(abertura, cv2.MORPH_CLOSE, k)
    erodido = cv2.erode(fechamento, k, iterations=1)

    contorno, _ = cv2.findContours(
        erodido.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
    secao_contorno_ordenada = sorted(contorno, key=lambda ctr: cv2.boundingRect(ctr)[1])

    PPimg_caractere = original.copy()

    for ctr in secao_contorno_ordenada:
        x, y, w, h = cv2.boundingRect(ctr)
        if w > 10 and h > 10:
            lista_palavra_seg.append([x, y, x + w, y + h])
            cv2.rectangle(PPimg_caractere, (x, y), (x + w, y + h), (100, 100, 255), 2)
    ...
    cv2.imwrite(dir_saida, PPimg_caractere)
